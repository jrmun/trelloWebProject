...
class ColumnService {
    ...
    getColumn = async () => {
        const getColumnData = await columnRepository.findAllColumn({
            order: [['createAt', 'DESC']],
        });
    ...
    createColumn = async (column_name, board_id, user_id) => {
        const columnData = await columnRepository.createColumn(column_name, board_id, user_id);

        return columnData;
    };

    updateColumn = async (column_id, column_name, board_id, user_id) => {
        const findColumn = await columnRepository.findColumnById(column_id);
